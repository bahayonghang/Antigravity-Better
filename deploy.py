...
TARGET_SEARCH_PATHS = [
    ...
    Path.home() / "Applications" / "Antigravity.app" / "Contents" / "Resources" / "app" / "out" / "vs" / "code" / "electron-browser" / "workbench",
    # Windows 路径
...
def format_permission_error(e: PermissionError, attempted_path: Path | None = None) -> str:
    """格式化权限错误提示，区分 sudo 和 macOS TCC/App Management 场景"""
    lines = [f"权限错误：{e}"]
    if attempted_path is not None:
        lines.append(f"失败路径：{attempted_path}")

    # macOS 在 /Applications/*.app 内写文件时，可能因 App Management 被拒绝。
    if sys.platform == "darwin" and e.errno == 1:
        lines.append("这通常不是 sudo 失效，而是 macOS 阻止当前终端修改 /Applications 下的 App Bundle。")
        lines.append("请为你当前使用的终端或 IDE 授予“应用管理（App Management）”权限后重试。")
        lines.append("路径：系统设置 -> 隐私与安全性 -> 应用管理")
        lines.append("如果你是在 IDE 内置终端里执行，也需要给该 IDE 本体授权。")
        return "\n".join(lines)

    lines.append("如目标目录在 /Applications，下次可尝试：sudo python3 deploy.py deploy")
    return "\n".join(lines)
    ...
    try:
        ...
        if not backup_file.exists():
            if dry_run:
                ...
                try:
                    ...
                    return False, format_permission_error(e, backup_file)
        ...
        if dry_run:
            ...
            try:
                ...
                return False, format_permission_error(e, target_file)
            print(f"✅ 已部署：{target_file}")

        return True, "部署成功！重启 Antigravity 生效。"

    except PermissionError as e:
        return False, format_permission_error(e)
